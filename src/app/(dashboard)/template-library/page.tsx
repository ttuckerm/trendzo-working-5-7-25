...
export default function TemplateLibraryPage() {
  ...
  const CanvasOverlay = useMemo(() => dynamic(() => import('@/components/canvas/CanvasOverlay'), { ssr: false }), []);

  useEffect(() => {
    // Restore deep-link into canvas hash if present
    if (typeof window !== 'undefined' && window.location.hash.startsWith('#canvas')) {
      const url = new URL('http://x/x' + window.location.hash.replace('#canvas', ''));
      const id = url.searchParams.get('templateId') || url.searchParams.get('t');
      if (id) setCanvasTemplateId(id);
    }
  }, []);

  function openCanvas(templateId: string) {
    setCanvasTemplateId(templateId);
    const params = new URLSearchParams();
    params.set('templateId', templateId);
    params.set('mode', 'reader');
    const next = `#canvas?${params.toString()}`;
    if (window.location.hash !== next) window.location.hash = next;
  }

  function closeCanvas() {
    setCanvasTemplateId(null);
    // Restore to previous hash-less URL
  ...
  return (
    <div className="container mx-auto">
            ...
            {filteredTemplates.length > 0 ? (
              filteredTemplates.map((template) => (
                <button
                  ...
                  onClick={(e) => { e.preventDefault(); e.stopPropagation(); openCanvas(template.id); }}
                >
      ...
      {canvasTemplateId && (
        <CanvasOverlay templateId={canvasTemplateId} onExit={closeCanvas} />
      )}
    </div>
  );
}
