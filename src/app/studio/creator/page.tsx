 ...
 * real-time VPS prediction as users progress through each step.
 */
...
function VPSScore({ value }: { value: number }) {
  const getGradient = () => {
...
function CreationHub({ data, onUpdate, onNext, liveVPS }: any) {
  ...
  return (
    <div className="space-y-8">
      <div className="text-center">
        ...
        <p className="text-white/60">Current VPS Score: <VPSScore value={liveVPS} /></p>
      </div>
...
export default function CreatorStudio() {
  ...
  const [liveVPS, setLiveVPS] = useState(50);
  const [predictions, setPredictions] = useState<any>({});
  const [completedSteps, setCompletedSteps] = useState<Record<string, boolean>>({});
  const [isCalculatingVPS, setIsCalculatingVPS] = useState(false);
  const [kaiComponents, setKaiComponents] = useState<string[]>([]);

  // REAL VPS calculation using Kai Orchestrator
  useEffect(() => {
    const calculateRealVPS = async () => {
      // Only call Kai if we have meaningful data
      const hasEnoughData = workflowData.niche || workflowData.topic || workflowData.script;

      if (!hasEnoughData) {
        // Show baseline VPS for early steps
        const completedCount = Object.keys(workflowData).length;
        const baseVPS = 40 + (completedCount * 2);
        setLiveVPS(baseVPS);
        return;
      }

      setIsCalculatingVPS(true);
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (result.success && result.prediction) {
            const realVPS = result.prediction.dps;
            setLiveVPS(realVPS);
            setPredictions(result.prediction);
            setKaiComponents(result.prediction.componentsUsed || []);

            console.log(`✅ Kai prediction: ${realVPS} VPS (${result.prediction.componentsUsed?.length || 0} components)`);

            // Dispatch event for navigation bar
            window.dispatchEvent(new CustomEvent('vps-update', {
              detail: {
                vps: realVPS,
          ...
          const completedCount = Object.keys(workflowData).length;
          const baseVPS = 40 + (completedCount * 3);
          setLiveVPS(Math.min(85, baseVPS));
        }
      } catch (error) {
        console.error('Error calling Kai:', error);
        // Fallback calculation
        const completedCount = Object.keys(workflowData).length;
        setLiveVPS(40 + (completedCount * 3));
      } finally {
        setIsCalculatingVPS(false);
      }
    };

    // Debounce the API call
    const timeoutId = setTimeout(calculateRealVPS, 500);
  ...
  return (
    <div className="min-h-screen bg-black pt-16">
      {/* Persistent VPS Bar - NOW CONNECTED TO KAI */}
      <div className="fixed top-16 left-0 right-0 z-40 bg-gradient-to-r from-purple-900/90 to-pink-900/90 backdrop-blur-sm border-b border-white/10">
        <div className="max-w-7xl mx-auto px-6 py-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <span className="text-white/80">
                {isCalculatingVPS ? '⏳ Analyzing with Kai...' : 'Live VPS Score:'}
              </span>
              <div className="text-3xl font-bold">
                {isCalculatingVPS ? (
                  <span className="text-white/50 animate-pulse">...</span>
                ) : (
                  <VPSScore value={liveVPS} />
                )}
              </div>
            ...
            <div className="flex gap-2">
              ...
              {!isCalculatingVPS && liveVPS < 70 && (
                <span className="px-3 py-1 bg-yellow-500/20 text-yellow-400 rounded-full text-sm">
                  Needs Optimization
                </span>
              )}
              {!isCalculatingVPS && liveVPS >= 70 && liveVPS < 85 && (
                <span className="px-3 py-1 bg-green-500/20 text-green-400 rounded-full text-sm">
                  ✨ Viral Potential
                </span>
              )}
              {!isCalculatingVPS && liveVPS >= 85 && (
          ...
          <div className="mt-2 h-2 bg-black/20 rounded-full overflow-hidden">
            <motion.div
              className={`h-full ${isCalculatingVPS ? 'bg-blue-500 animate-pulse' : 'bg-gradient-to-r from-green-500 to-emerald-500'}`}
              initial={{ width: 0 }}
              animate={{ width: `${liveVPS}%` }}
              transition={{ duration: 0.5 }}
            />
          </div>
          {/* Show Kai components when available */}
          {kaiComponents.length > 0 && !isCalculatingVPS && (
