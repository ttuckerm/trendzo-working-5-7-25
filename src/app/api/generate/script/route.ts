...
interface ScriptGenerationRequest {
  ...
  /** Creator calibration: preferred hook styles (key→score map) */
  hookStylePreference?: Record<string, number>;
  /** Creator calibration: preferred tones (key→score map) */
  toneMatch?: Record<string, number>;
  /** Pattern Library saturation info for the selected pattern */
  patternSaturation?: { pattern_name: string; saturation_pct: number; lifecycle_stage: string };
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { concept, platform, length, niche, patternId, hookStylePreference, toneMatch, patternSaturation } = body;
    ...
    const scriptResult = await generateScriptWithOpenAI(
      ...
      platformGuidance,
      patternData,
      { hookStylePreference, toneMatch, patternSaturation }
    );
...
CRITICAL: Your script MUST follow the hook structure and story pattern from the PRIMARY VIRAL PATTERN above. Don't just make something up - USE THE PROVEN FORMULA.`
  + buildCreatorPrefsSection(creatorPrefs);
...
/**
 * Build an optional "CREATOR PREFERENCES" section for the system prompt.
 * Returns empty string when no preferences are available (no regression).
 */
function buildCreatorPrefsSection(prefs?: {
  hookStylePreference?: Record<string, number>;
  toneMatch?: Record<string, number>;
  patternSaturation?: { pattern_name: string; saturation_pct: number; lifecycle_stage: string };
}): string {
  ...
  // Top 3 hook styles
  if (prefs.hookStylePreference && Object.keys(prefs.hookStylePreference).length > 0) {
    const sorted = Object.entries(prefs.hookStylePreference)
      .sort(([, a], [, b]) => b - a)
      .slice(0, 3)
      .map(([key]) => key.replace(/-/g, ' '));
    parts.push(`- Preferred hook styles: ${sorted.join(', ')}`);
  }

  // Top 3 tones
  if (prefs.toneMatch && Object.keys(prefs.toneMatch).length > 0) {
    const sorted = Object.entries(prefs.toneMatch)
      .sort(([, a], [, b]) => b - a)
      .slice(0, 3)
      .map(([key]) => key.replace(/-/g, ' '));
    parts.push(`- Preferred tone: ${sorted.join(', ')}`);
  }

  if (parts.length === 0 && !prefs.patternSaturation) return '';

  let section = '\n\n**CREATOR PREFERENCES (match their style)**:\n' + parts.join('\n');

  // Saturation warning
  if (prefs.patternSaturation && prefs.patternSaturation.saturation_pct > 60) {
    section += `\n\n**WARNING**: The pattern "${prefs.patternSaturation.pattern_name}" is becoming saturated (${prefs.patternSaturation.saturation_pct.toFixed(0)}%). Suggest a unique twist or angle that hasn't been done yet to differentiate from the crowd.`;
  }
