...
## Their Freedom OS Plan
- Business type: ${plan.preferredBusiness || 'not specified'}
- Niche interest: ${plan.nicheInterest || 'not specified'}
- Available hours/week: ${plan.availableHours || 'not specified'}
- Target income: ${plan.targetIncome || 'not specified'}
- Experience level: ${plan.comfortLevel || 'not specified'}
- Risk tolerance: ${plan.riskTolerance || 'not specified'}
- Current savings runway: ${plan.savingsRunway || 'not specified'}

Use this plan to make your advice specific, not generic. Reference their actual numbers and goals.
` : `
## No plan available
The user hasn't completed Freedom OS yet. Encourage them to try it at /free/freedom-os to get personalized guidance, but still provide helpful general advice.
`;
  ...
  const nicheTemplate = getNicheTemplate(plan?.nicheInterest, plan?.preferredBusiness);

  // Week-based soft-sell arc directive
  const weekDirective = getWeekDirective(
    session.current_week,
    session.progress_state || {},
    plan?.nicheInterest || null
  );
