  ...
  try {
    ...
    for (const check of pendingChecks) {
      try {
        ...
        // Calculate DPS
        const dps = calculateEngagementRate(metrics.views, metrics.likes, metrics.comments, metrics.shares, metrics.saves || 0);
...
// Tracking checks now store raw engagement rate instead of a legacy DPS score.
function calculateEngagementRate(views: number, likes: number, comments: number, shares: number, saves: number): number {
  if (views === 0) return 0;
