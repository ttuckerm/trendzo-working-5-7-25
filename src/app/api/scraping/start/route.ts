...
async function processScraping(
  ...
  try {
    ...
    for (const video of filteredVideos) {
      try {
        const actualDps = calculateDPS(video);
 ...
 * EVIDENCE: src/lib/services/dps/dps-calculation-engine.ts exists
 */
function calculateDPS(video: any): number {
  const metrics = video.metrics?.[0];
  if (!metrics) return 0;

  const { views = 0, likes = 0, shares = 0, comments = 0 } = metrics;

  // Simplified DPS formula
  const engagement = (likes + shares * 2 + comments * 3) / Math.max(views, 1);
  const dps = Math.min(engagement * 10000, 100); // Scale to 0-100
...
async function analyzePatterns(jobId: string, videos: any[]) {
  ...
  const viral = videos.filter(v => calculateDPS(v) >= 70);
  const good = videos.filter(v => calculateDPS(v) >= 50 && calculateDPS(v) < 70);
  const poor = videos.filter(v => calculateDPS(v) < 50);
