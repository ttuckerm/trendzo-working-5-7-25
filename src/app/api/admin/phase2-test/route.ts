...
export async function GET(request: NextRequest) {
  ...
  // Test 2: DPS v2 module
  try {
    const { DPS_V2_FORMULA_VERSION } = await import('@/lib/training/dps-v2');
    testResults.total_services++;
    testResults.tests.push({
      service: 'dps-v2',
      status: DPS_V2_FORMULA_VERSION === '2.1.0' ? 'success' : 'warning',
      test: 'dps_v2_version',
      message: `DPS v2 formula version: ${DPS_V2_FORMULA_VERSION}`
    });
    if (DPS_V2_FORMULA_VERSION === '2.1.0') testResults.successful_services++;
  } catch (error) {
    testResults.total_services++;
    testResults.tests.push({
      service: 'dps-v2',
      status: 'error',
      test: 'dps_v2_import_failed',
      error: error instanceof Error ? error.message : 'Unknown error'
    });
  }
