...
// Bulk imports now store raw metrics only — v2 uses them for cohort stats.
...
function normalizeApifyVideo(video: ApifyVideoData, niche: string, source: string): Record<string, unknown> {
  ...
  // DPS scoring removed (2026-03-25) — raw metrics stored; v2 uses cohort stats.
  const dps = null;
  ...
  return {
    ...
    pattern_extraction_status: 'not_required',
