...
export async function POST(req: NextRequest) {
  ...
  if (!parsed.success) {
    ...
    try {
      await getRedis().hincrby('dlq:ingest:reasons', 'schema', 1)
      await getRedis().lpush('dlq:ingest:entries', JSON.stringify({ module: 'ingest', reason: 'schema', firstSeenAt: new Date().toISOString(), count: 1, sample: json }))
      await getRedis().ltrim('dlq:ingest:entries', 0, 199)
    } catch {}
    return NextResponse.json({ error: 'invalid payload', details: parsed.error.flatten() }, { status: 400 })
  }
  const { vit } = parsed.data
  // Circuit breaker: pause if flagged
  const paused = await getRedis().get('controls:ingest:paused')
  ...
  if (process.env.LOCAL_UPSERT === '1') {
    try {
      ...
      await getRedis().hset('local:upserts', key, JSON.stringify({ ...vit, vitVersion: parsed.data.vitVersion }))
    } catch {}
  ...
  // Update metrics
  await getRedis().decrby('metrics:ingest:queued', 1)
  await getRedis().incrby('metrics:ingest:completed', 1)
  await getRedis().set('metrics:ingest:rate_per_min', Math.max(1, Math.floor(Math.random() * 20)))
...
export async function GET() {
  if (process.env.LOCAL_UPSERT === '1') {
    try {
      const all = await getRedis().hgetall('local:upserts')
