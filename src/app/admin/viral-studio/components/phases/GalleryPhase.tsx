...
import { ChevronLeft, ChevronRight, Eye, Heart, Share, Play, Flame, Music, Sparkles, Loader2 } from 'lucide-react';
...
// Helper to calculate DPS from video metrics
  ...
  const fetchRealTemplates = async (category: string) => {
    ...
    try {
      ...
      const templates: Template[] = data.map((video, index) => {
        const dps = video.dps_score ? parseFloat(video.dps_score) : calculateDPS(video);
        ...
        return {
          ...
          viralScore: Math.round(dps * 10) / 10,
          description: video.caption || 'Real viral video from TikTok',
          trendingSound: `DPS: ${dps.toFixed(1)} - Real Performance Data`,
