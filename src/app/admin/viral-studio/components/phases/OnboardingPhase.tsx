...
interface OnboardingPhaseProps {
  onComplete: (niche: string, goal: string, nicheKey: string, subtopics?: string[]) => void;
...
const NICHE_OPTIONS: DropdownOption[] = [
  { key: 'personal-finance-investing', label: 'Personal Finance/Investing', description: 'Money management, investing tips', registryKey: 'personal-finance' },
  { key: 'fitness-weight-loss', label: 'Fitness/Weight Loss', description: 'Workouts, weight management, health', registryKey: 'fitness' },
  { key: 'business-entrepreneurship', label: 'Business/Entrepreneurship', description: 'Business tips, startup advice', registryKey: 'business' },
  { key: 'food-nutrition-comparisons', label: 'Food/Nutrition Comparisons', description: 'Food reviews, nutrition facts', registryKey: 'food-nutrition' },
  { key: 'beauty-skincare', label: 'Beauty/Skincare', description: 'Makeup, skincare routines, beauty tips', registryKey: 'beauty' },
  { key: 'real-estate-property', label: 'Real Estate/Property', description: 'Property investment, real estate tips', registryKey: 'real-estate' },
  { key: 'self-improvement-productivity', label: 'Self-Improvement/Productivity', description: 'Personal development, productivity hacks', registryKey: 'self-improvement' },
  { key: 'dating-relationships', label: 'Dating/Relationships', description: 'Dating advice, relationship tips', registryKey: 'dating' },
  { key: 'education-study-tips', label: 'Education/Study Tips', description: 'Learning strategies, academic advice', registryKey: 'education' },
  { key: 'career-job-advice', label: 'Career/Job Advice', description: 'Career development, job search tips', registryKey: 'career' },
  { key: 'parenting-family', label: 'Parenting/Family', description: 'Parenting tips, family activities', registryKey: 'parenting' },
  { key: 'tech-reviews-tutorials', label: 'Tech Reviews/Tutorials', description: 'Technology reviews, tech tutorials', registryKey: 'tech' },
  { key: 'fashion-style', label: 'Fashion/Style', description: 'Fashion trends, styling tips', registryKey: 'fashion' },
  { key: 'health-medical-education', label: 'Health/Medical Education', description: 'Health information, medical education', registryKey: 'health' },
  { key: 'cooking-recipes', label: 'Cooking/Recipes', description: 'Cooking tips, recipe tutorials', registryKey: 'cooking' },
  { key: 'psychology-mental-health', label: 'Psychology/Mental Health', description: 'Mental wellness, psychology insights', registryKey: 'psychology' },
  { key: 'travel-lifestyle', label: 'Travel/Lifestyle', description: 'Travel tips, lifestyle content', registryKey: 'travel' },
  { key: 'diy-home-improvement', label: 'DIY/Home Improvement', description: 'DIY projects, home renovation', registryKey: 'diy' },
  { key: 'language-learning', label: 'Language Learning', description: 'Language tutorials, learning tips', registryKey: 'language' },
  { key: 'side-hustles-making-money-online', label: 'Side Hustles/Making Money Online', description: 'Online income, side business ideas', registryKey: 'side-hustles' }
];
...
export default function OnboardingPhase({ onComplete, selectedNiche: initialSelectedNiche, selectedGoal: initialSelectedGoal }: OnboardingPhaseProps) {
  ...
  const getSubtopicsForNiche = (): string[] => {
    const nicheData = NICHE_OPTIONS.find(n => n.key === selectedNiche);
    const registryEntry = NICHE_REGISTRY.find(n => n.key === nicheData?.registryKey);
    return registryEntry?.subtopics ?? [];
  };

  const toggleSubtopic = (subtopic: string) => {
    setSelectedSubtopics(prev => {
      if (prev.includes(subtopic)) return prev.filter(s => s !== subtopic);
      if (prev.length >= 5) return prev;
      return [...prev, subtopic];
    });
  };

  const handleSubtopicsContinue = () => {
    setStep(3);
  };
  ...
  const handleGoalSelect = (goal: DropdownOption) => {
    ...
    setTimeout(() => {
      ...
      onComplete(
        selectedNicheData?.label || selectedNiche,
        goal.label,
        selectedNicheData?.registryKey || selectedNiche,
    ...
    >
      <div className="text-center max-w-3xl w-full">
            ...
            >
              <div className="mb-6">
                ...
                <p className="text-white/60 text-sm">
                  Pick 3-5 subtopics ({selectedSubtopics.length}/5 selected)
                </p>
              </div>

              <div className="max-w-lg mx-auto">
                <div className="flex flex-wrap justify-center gap-2 mb-8">
                  {getSubtopicsForNiche().map((subtopic) => {
                    const isSelected = selectedSubtopics.includes(subtopic);
                    const isDisabled = !isSelected && selectedSubtopics.length >= 5;
                    return (
                      <motion.button
                        key={subtopic}
                        onClick={() => !isDisabled && toggleSubtopic(subtopic)}
                        className={`
                          px-4 py-2 rounded-full border text-sm transition-all duration-200
                          ${isSelected
                            ? 'bg-[#7b61ff]/20 border-[#7b61ff]/60 text-[#7b61ff]'
                            : isDisabled
                              ? 'bg-white/[0.02] border-white/10 text-white/30 cursor-not-allowed'
                              : 'bg-white/[0.03] border-white/20 text-white/70 hover:border-white/40 hover:text-white'
                          }
                        `}
                ...
                <motion.button
                  onClick={handleSubtopicsContinue}
                  ...
                  className={`
                    ...
                    ${selectedSubtopics.length >= 3
                      ? 'bg-gradient-to-r from-[#7b61ff] to-[#ff61a6] text-white hover:shadow-lg hover:shadow-[#7b61ff]/25'
                      : 'bg-white/10 text-white/30 cursor-not-allowed'
                    }
                  `}
                  whileHover={selectedSubtopics.length >= 3 ? { scale: 1.05 } : {}}
                  whileTap={selectedSubtopics.length >= 3 ? { scale: 0.95 } : {}}
                >
                  Continue
                </motion.button>
              </div>
        ...
        >
          ...
          <div className={`w-3 h-3 rounded-full transition-all duration-300 ${step >= 3 ? 'bg-[#7b61ff]' : 'bg-white/20'}`} />
        </motion.div>
      </div>
    </motion.div>
  );
