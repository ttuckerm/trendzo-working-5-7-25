  ...
  useEffect(() => {
    ...
    handlerRef.current = (e: KeyboardEvent) => {
      if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === 'j') {
        if (!visible) {
          setVisible(true)
          // Focus input after a brief delay to ensure the overlay is rendered
          setTimeout(() => {
    ...
    const h = async ()=>{
      try { const r = await fetch('/api/admin/jarvis/status', { cache: 'no-store' }); const j = await r.json(); setStatus(j) } catch {}
    }
    
    window.addEventListener('keydown', handlerRef.current)
    const interval = setInterval(h, 3000)
    h()
    
    // Check mic status on mount and when overlay becomes visible
    if (visible) {
      checkMicStatus()
    }
