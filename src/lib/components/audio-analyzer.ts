...
export class AudioAnalyzer {
  ...
  /**
   * Convert audio analysis to DPS prediction
   */
  public static toDPS(result: AudioAnalysisResult): number {
    if (!result.success) {
      return 50; // Neutral if analysis failed
    }

    // Map audio score (0-10) to DPS (35-75)
    // Audio is important but not the primary factor
    const baseDPS = 35 + (result.audioScore * 4);
    ...
    const finalDPS = Math.min(75, Math.max(35, baseDPS + boost));
