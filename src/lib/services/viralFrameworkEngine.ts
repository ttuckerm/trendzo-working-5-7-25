// ViralFrameworkEngine — used by template/pattern engines.
// calculateViralScore() uses hardcoded mock data for display only.
// Ground-truth DPS scoring is in src/lib/training/dps-v2.ts.

import type { Platform, Niche } from '@/lib/types/database';
...
export class ViralFrameworkEngine {
  ...
  calculateViralScore(params: {
    ...
    const platformWeight = this.getPlatformWeight(params.platform);
    const decayRate = this.getPlatformSpecs(params.platform).decayRate;
    const decayFactor = Math.exp(-decayRate * params.timeElapsed);
    const viralScore = (params.viewCount / cohortMedian) * platformWeight * decayFactor;
    ...
    if (viralScore >= 25.0) {
      ...
      percentile = 99.9;
    } else if (viralScore >= 10.0) {
      classification = 'hyper_viral';
      percentile = 99;
    } else if (viralScore >= 5.0) {
