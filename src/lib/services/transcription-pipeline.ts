...
import { transcribeVideo } from './whisper-service';
  ...
  if (options.videoPath && fs.existsSync(options.videoPath)) {
    ...
    try {
      ...
      if (whisperText.length >= WHISPER_EMPTY_THRESHOLD && wordCount >= MIN_WHISPER_WORD_COUNT) {
        console.log(`[TranscriptionPipeline] Whisper success: ${whisperText.length} chars, ${wordCount} words`);

        // Calculate confidence based on word count and length
        const confidence = calculateWhisperConfidence(whisperText, wordCount);
