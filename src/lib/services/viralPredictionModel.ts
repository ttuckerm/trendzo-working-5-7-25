...
import { saveModel } from '@/lib/services/model-artifacts';
...
export class ViralPredictionModel {
  ...
  async trainModel(
    ...
    try {
      ...
      // Persist artifacts and experiment run
      try {
        await saveModel(platform, modelId, { weights: modelResult.weights, meta: performance });
        await supabaseClient.from('experiment_runs').insert({
          platform,
        ...
        console.warn('Artifact/experiment logging failed:', e);
      }
