...
// Initialize Gemini with API key - check multiple env var names
// Priority: GOOGLE_GEMINI_AI_API_KEY (paid tier) > GOOGLE_AI_API_KEY > GEMINI_API_KEY
const geminiApiKey = process.env.GOOGLE_GEMINI_AI_API_KEY || 
...
export class GeminiVideoAnalyzer {
  ...
  constructor(modelName: string = 'gemini-2.0-flash-exp') {
    // Use Gemini 2.0 Flash for multimodal capability (free tier available)
    // Alternative models: 'gemini-1.5-flash', 'gemini-1.5-pro', 'gemini-3-pro-preview'
  ...
  async analyzeVideo(videoPath: string): Promise<GeminiVideoAnalysis> {
    try {
      ...
      // Send to Gemini 3.0 Pro with video
      ...
      if (error.message?.includes('not found') || error.status === 404) {
        errorMessage = `Model "${this.modelName}" not found. Try: gemini-2.0-flash-exp, gemini-1.5-pro, or gemini-1.5-flash`;
      }
