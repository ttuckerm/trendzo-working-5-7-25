 ...
 *
 * Rule-based visual analysis that runs even when transcript is missing.
...
export async function runVisualRubric(input: VisualRubricInput): Promise<VisualRubricResult> {
  ...
  const overallScore = Math.round(
    (visualHookScore.score * weights.visualHook +
      pacingScore.score * weights.pacing +
      patternInterruptsScore.score * weights.patternInterrupts +
      visualClarityScore.score * weights.visualClarity +
      styleFitScore.score * weights.styleFit) *
      10 // Scale to 0-100
  );
  ...
  const result: VisualRubricResult = {
    pack: 'V',
    visual_hook_score: visualHookScore,
    pacing_score: pacingScore,
    pattern_interrupts_score: patternInterruptsScore,
    visual_clarity_score: visualClarityScore,
    style_fit_score: styleFitScore,
  ...
  try {
    ...
    console.log(`[VisualRubric] Analysis complete: overall=${overallScore}/100 in ${latencyMs}ms`);
