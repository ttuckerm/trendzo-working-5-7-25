...
function detectVisualHook(
  ...
  // Check Pack V visual scores
  if (packV?.visual_scores) {
    const hookScore = packV.visual_scores.find((s: any) => s.dimension === 'hook_strength');
    if (hookScore && hookScore.score >= 7) {
      strength = Math.max(strength, hookScore.score * 10);
      evidence.push(`Strong visual hook detected (score: ${hookScore.score}/10)`);
      signals.push('packV.hook_strength');
    }
  }
...
function detectOptimalPacing(pack1: any | null, packV: any | null): ViralMechanic | null {
  ...
  // Check Pack V for visual pacing
  if (packV?.visual_scores) {
    const pacingScore = packV.visual_scores.find((s: any) =>
      s.dimension === 'pacing' || s.dimension === 'editing_rhythm'
    );
    if (pacingScore && pacingScore.score >= 7) {
      strength = Math.max(strength, pacingScore.score * 10);
      evidence.push(`Visual pacing score: ${pacingScore.score}/10`);
      signals.push('packV.pacing');
    }
  }
...
function detectTrendAlignment(
  ...
  // Check trend-timing component
  const trendTiming = componentMap.get('trend-timing');
  if (trendTiming?.features) {
    const features = trendTiming.features;
    if (features.trend_score) {
      strength = Math.max(strength, features.trend_score);
      evidence.push(`Trend alignment score: ${features.trend_score}/100`);
      signals.push('trend-timing');
    }
    if (features.trending_topic) {
      evidence.push(`Aligns with trending topic: "${features.trending_topic}"`);
    }
  }

  // Check historical-analyzer
  const historical = componentMap.get('historical-analyzer');
  if (historical?.features?.similar_viral_videos) {
    strength = Math.max(strength, 60);
    evidence.push('Similar content has performed well historically');
    signals.push('historical-analyzer');
  }

  // Check virality-matrix
  const viralityMatrix = componentMap.get('virality-matrix');
  if (viralityMatrix?.prediction && viralityMatrix.prediction > 60) {
    strength = Math.max(strength, viralityMatrix.prediction);
    evidence.push(`Virality matrix prediction: ${viralityMatrix.prediction}/100`);
    signals.push('virality-matrix');
  }

  if (strength >= 50) {
    return {
      name: VIRAL_MECHANICS.TREND_ALIGNMENT,
      strength: Math.round(strength),
      evidence,
      signals_used: signals,
    };
...
function detectPatternInterrupt(packV: any | null, pack1: any | null): ViralMechanic | null {
  ...
  // Check Pack V for visual complexity/interest
  if (packV?.visual_scores) {
    const noveltyScore = packV.visual_scores.find((s: any) =>
      s.dimension === 'novelty' || s.dimension === 'uniqueness' || s.dimension === 'visual_interest'
    );
    if (noveltyScore && noveltyScore.score >= 7) {
      strength = Math.max(strength, noveltyScore.score * 10);
      evidence.push(`High visual novelty (score: ${noveltyScore.score}/10)`);
      signals.push('packV.novelty');
    }
  }
...
function detectEmotionalTrigger(
  ...
  if (pack1?.attribute_scores) {
    const emotionalAttrs = ['emotional_resonance', 'relatability', 'entertainment_value'];
...
function detectTimingAdvantage(
  ...
  // Check posting-optimizer component
  const postingOptimizer = componentMap.get('posting-optimizer');
  if (postingOptimizer?.features) {
    const features = postingOptimizer.features;
    if (features.optimal_time) {
      strength = 60;
      evidence.push(`Recommended posting time: ${features.optimal_time}`);
      signals.push('posting-optimizer');
    }
    if (features.timing_score) {
      strength = Math.max(strength, features.timing_score);
      evidence.push(`Timing optimization score: ${features.timing_score}/100`);
    }
  }

  if (strength >= 50) {
    return {
      name: VIRAL_MECHANICS.TIMING_ADVANTAGE,
      strength: Math.round(strength),
      evidence,
      signals_used: signals,
    };
