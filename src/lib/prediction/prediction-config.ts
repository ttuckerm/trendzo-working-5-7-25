...
export function checkComponentInputs(
  ...
  requiredInputs: Array<'transcript' | 'videoPath' | 'ffmpegData' | 'either'>
) {
  ...
  for (const requiredInput of requiredInputs) {
    if (requiredInput === 'either') {
      if (!input.transcript && !input.videoPath && !input.ffmpegData) {
        return {
          shouldRun: false,
          skipReason: 'Missing required input: need at least transcript, videoPath, or ffmpegData',
        };
      }
    } else if (!input[requiredInput]) {
      return { 
        shouldRun: false, 
