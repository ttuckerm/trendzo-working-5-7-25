...
// Imperative toast for non-React contexts (error boundaries, services)
export function showToast({ title, description, variant = "default", duration = 3000 }: ToastProps) {
  ...
  window.setTimeout(() => {
    ...
    toastEl.classList.add("translate-y-2");
    window.setTimeout(() => {
      try { toastContainer?.removeChild(toastEl); } catch {}
      if (toastContainer && toastContainer.childElementCount === 0) {
        try { document.body.removeChild(toastContainer); } catch {}
      }
    }, 500);
  }, duration);
}