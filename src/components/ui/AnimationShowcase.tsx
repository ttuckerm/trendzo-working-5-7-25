...
export default function AnimationShowcase() {
  ...
  // Example: Convert animationSpeed to a numeric scale for demonstration
  const currentAnimationScale = animationSpeed === 'fast' ? 0.5 : animationSpeed === 'slow' ? 1.5 : 1.0;
          ...
          <div className="flex flex-col space-y-4 sm:flex-row sm:space-y-0 sm:space-x-4 mb-8">
            <div className="p-4 border rounded-lg bg-white shadow-sm">
              ...
              <div className="flex items-center justify-between mb-2">
                <label htmlFor="reduced-motion-toggle" className="mr-4">
                  Reduced Motion:
                </label>
                <div className="form-switch">
                  ...
                  />
                  <div className={`switch-toggle ${isReducedMotion ? 'bg-primary-500' : 'bg-neutral-300'}`}></div>
                </div>
              </div>
              
              <div className="flex items-center justify-between mb-2">
                <label htmlFor="advanced-animations-toggle" className="mr-4">
                  Advanced Animations:
                </label>
                <div className="form-switch">
                  <input
                    type="checkbox"
                    id="advanced-animations-toggle"
                    className="sr-only"
                    checked={useAdvancedAnimations}
                    onChange={toggleAdvancedAnimations}
                  />
                  <div className={`switch-toggle ${useAdvancedAnimations ? 'bg-primary-500' : 'bg-neutral-300'}`}></div>
                </div>
              </div>
              
              <div className="mt-4">
                <label htmlFor="animation-speed" className="block mb-2">
                  Animation Speed: {animationSpeed} (Scale: {currentAnimationScale})
                </label>
                <select
                  id="animation-speed"
              ...
              {isReducedMotion && (
                <div className="mt-4 p-2 bg-yellow-100 text-yellow-800 rounded text-sm">
                  Reduced motion preference is active. Animations are minimized.
                </div>
              )}
            </div>
          </div>
