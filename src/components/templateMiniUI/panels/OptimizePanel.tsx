...
import { CheckCircle, AlertTriangle, Zap, Loader2, ArrowUp } from "lucide-react";
    ...
    <div className="space-y-4" role="tabpanel" aria-label="Optimize panel">
      ...
      <div className="p-3 rounded-lg border bg-card text-card-foreground">
        <div className="flex items-center justify-between mb-2">
          ...
          <Badge variant={potentialIssues.length === 0 ? "default" : "secondary"}>
            {potentialIssues.length === 0 ? "Perfect" : `${Math.max(0, 10 - potentialIssues.length * 2)}/10`}
          </Badge>
        </div>
        <div className="text-xs text-muted-foreground">
          {potentialIssues.length === 0 
            ? "Your content is fully optimized!" 
            : `${potentialIssues.length} optimization${potentialIssues.length > 1 ? 's' : ''} available`
          }
        </div>
      </div>
