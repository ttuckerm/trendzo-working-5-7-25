...
export const TemplateEditorProvider: React.FC<TemplateEditorProviderProps> = ({
  ...
  // Effect to load recipe data when initialRecipe changes
  useEffect(() => {
    if (initialRecipe && initialRecipe.template_data) {
      // Extract template data from the recipe
      const recipeData = initialRecipe.template_data;
      
      // Create a new section with elements based on the recipe patterns
      const newSection: TemplateSection = {
        id: generateId(),
      ...
      newSection.elements.push(titleElement);
      
      // Add description element
      if (initialRecipe.description) {
        ...
        newSection.elements.push(descElement);
      }
      
      // Add pattern elements if available
      if (recipeData.patterns && Array.isArray(recipeData.patterns)) {
        recipeData.patterns.forEach((pattern: any, index: number) => {
          const patternElement: Element = {
            id: generateId(),
            type: "text",
            content: `${pattern.type === 'sound' ? '🎵' : pattern.type === 'hashtag' ? '#' : '🔑'} ${pattern.value}`,
            x: 50,
            y: 220 + (index * 40),
            width: 300,
            height: 30,
          ...
          newSection.elements.push(patternElement);
        });
      }
      
      // Update the template with the new section
      dispatch({
