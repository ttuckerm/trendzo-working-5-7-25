...
## Milestone: v2.0 - Component Algorithmic Conversion (Post-Audit)

**Goal**: Transform the prediction system from "LLM opinion ensemble" to "deterministic measurement engine with LLM refinement"

**Status**: Framework established, execution pending

**Context**: God's Eye forensic audit (Mar 5-7) revealed 73.5% accuracy is fabricated, VPS is dominated by correlated LLM opinions, and XGBoost v6 is broken. Components must become deterministic before model training is meaningful.

---

### Phase A: Component-by-Component Audit
**Status**: In Progress (Component 1 next)
**Description**: Audit each of 22 components using concept → reality → feasibility framework

**Deliverables**:
- [ ] Component 1: FFmpeg Video Analysis — deep-dive
- [ ] Components 2-22: systematic audit
- [ ] For each: keep/remove/fix/merge verdict with algorithmic vs. LLM boundary

---

### Phase B: Deterministic Signal Pipeline
**Status**: Pending (blocked on Phase A)
**Description**: Convert LLM-dependent components to deterministic measurements

**Deliverables**:
- [ ] Hook analysis: algorithmic (word count, question detection, scene changes) + LLM classification
- [ ] Audio analysis: complete FFmpeg extraction (no placeholders)
- [ ] Text analysis: NLP metrics (readability, sentiment, vocabulary)
- [ ] Visual analysis: frame-level features via FFmpeg
- [ ] Style classification: deterministic rules replacing broken component

---

### Phase C: Wire Signals to Scoring
**Status**: Pending (blocked on Phase B)
**Description**: Connect deterministic features to scoring formula

**Deliverables**:
- [ ] Feature extraction feeds into XGBoost input
- [ ] Weights derived from data (not hardcoded)
- [ ] Consensus gate limited to true qualitative evaluators (3, not 7)
- [ ] Remove/fix Rule 4 compression based on real data
- [ ] Historical dimension connected or formally removed

---

### Phase D: Model Training
**Status**: Blocked (need 100+ labeled videos)
**Description**: Train model on deterministic signals + real outcomes

**Deliverables**:
- [ ] 100+ labeled prediction runs with actual_dps
- [ ] XGBoost v7 retrain on real features
- [ ] Real Spearman ρ baseline established
- [ ] Within-niche ranking evaluation
- [ ] Component weight optimization from data
