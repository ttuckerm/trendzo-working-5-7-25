-- ============================================
-- BRIDGE TABLES TO CONNECT EXISTING & NEW DATA
...
CREATE TABLE IF NOT EXISTS template_video_analysis (
    ...
    template_id UUID, -- Reference to your existing templates table
    tiktok_template_id UUID, -- Reference to your tiktok_templates table
...
CREATE TABLE IF NOT EXISTS user_viral_preferences (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    user_id UUID, -- Reference to your existing users table
...
CREATE TABLE IF NOT EXISTS sound_viral_history (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    sound_id UUID, -- Reference to your existing sounds table
...
CREATE TABLE IF NOT EXISTS expert_insight_enhancements (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    expert_insight_id UUID, -- Reference to your existing expert_insights table
...
-- 1. Complete Video Analysis View
...
LEFT JOIN sounds s ON v.sound_id::UUID = s.id
...
SELECT 
    ...
    ROUND(
        (COUNT(CASE WHEN v.cohort_percentile >= 95 THEN 1 END) * 100.0 / COUNT(vh.*)), 2
...
BEGIN
    ...
    RAISE NOTICE '🔗 Your existing data can now connect to viral prediction system';
    RAISE NOTICE '📊 New views created for easy analysis';
    RAISE NOTICE '⚡ Helper functions available for data migration';
    RAISE NOTICE '';
    RAISE NOTICE 'Next steps:';
    RAISE NOTICE '1. Run: SELECT sync_tiktok_templates_to_videos(); to migrate existing data';
    RAISE NOTICE '2. Use views like "video_complete_analysis" for comprehensive insights';
    RAISE NOTICE '3. Your existing templates/analytics tables now have viral prediction columns';
    RAISE NOTICE '';
END $$;