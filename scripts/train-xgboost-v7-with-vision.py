# -*- coding: utf-8 -*-
"""
Retrain XGBoost v7 Model with 77 features (68 original + 5 FFmpeg segment + 4 Vision hook)
...
Reports Spearman delta vs the 73-feature baseline.
...
# Original 68 + 5 FFmpeg segment + 4 Vision hook = 77 total
V7_FEATURES = [
...
def main():
    print('=' * 60)
    print('  XGBoost v7 Retraining — 77 features (73 + 4 Vision hook)')
    ...
    # Check vision feature coverage
    for vf in VISION_FEATURES:
        if vf in df_train.columns:
            non_null = df_train[vf].notna().sum()
            print(f'    {vf}: {non_null}/{len(df_train)} non-null ({non_null/len(df_train)*100:.1f}%)')
        else:
            print(f'    {vf}: NOT IN COLUMNS')
    ...
    # Load 73-feature baseline for comparison
    ...
    print(f'\n  Performance ({len(present)} features vs 73 baseline):')
    print(f'                       Train       Holdout     73-feat baseline')
    ...
    # Show vision feature rankings
    print(f'\n  Vision hook feature rankings:')
    for vf in VISION_FEATURES:
        if vf in importance['feature'].values:
            imp_val = importance[importance['feature'] == vf]['importance'].values[0]
            rank_pos = list(importance['feature']).index(vf) + 1
            print(f'    {vf:<40} rank #{rank_pos}, importance={imp_val:.4f}')
    ...
    metadata = {
        ...
        'changes': f'Added 4 Vision hook features to 73-feature baseline ({len(present)} total)',
        'holdout_split': True,
        'holdout_count': len(holdout_ids),
        'holdout_file': 'models/holdout-video-ids.json',
        'feature_count': len(present),
        'feature_names': present,
        'new_features': VISION_FEATURES,
        'dataset': {
            'total_rows': len(df),
        ...
        'new_feature_importance': [
            {'feature': vf, 'importance': float(importance[importance['feature'] == vf]['importance'].values[0])}
            for vf in VISION_FEATURES if vf in importance['feature'].values
        ],
    }
    ...
    print(f'\n{"=" * 60}')
    print(f'  XGBoost v7 retraining complete ({len(present)} features).')
    ...
    if baseline_holdout_spearman:
        print(f'  Delta vs 73-feat:  {holdout_spearman - baseline_holdout_spearman:+.4f}')
    print(f'{"=" * 60}')
