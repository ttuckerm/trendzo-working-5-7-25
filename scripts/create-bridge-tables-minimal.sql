-- ============================================
-- MINIMAL BRIDGE TABLES (NO COLUMN ASSUMPTIONS)
-- Creates connections between existing and new data without assuming column names
-- ============================================
...
-- Extend analytics table with God Mode metrics (if it exists)
DO $$
BEGIN
    -- Check if analytics table exists first
...
CREATE TABLE IF NOT EXISTS template_video_analysis (
    ...
    template_id BIGINT, -- Reference to your existing templates table
    tiktok_template_id BIGINT, -- Reference to your tiktok_templates table
...
CREATE TABLE IF NOT EXISTS user_viral_preferences (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    user_id BIGINT, -- Reference to your existing users table
...
CREATE TABLE IF NOT EXISTS sound_viral_history (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    sound_id BIGINT, -- Reference to your existing sounds table
...
CREATE TABLE IF NOT EXISTS expert_insight_enhancements (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    expert_insight_id BIGINT, -- Reference to your existing expert_insights table
...
-- ============================================
-- CREATE SIMPLE VIEWS (NO COLUMN ASSUMPTIONS)
-- ============================================

-- 1. Viral Performance Dashboard View (videos only)
...
SELECT 
    ...
    COALESCE(pp.duet_potential_score, 0) + COALESCE(pp.stitch_potential_score, 0) as participation_score,
    ct.timing_score,
    ...
    ROUND(
        (COALESCE(pe.emotional_arousal_score, 0) * 0.20 +
         ...
         COALESCE((COALESCE(pp.duet_potential_score, 0) + COALESCE(pp.stitch_potential_score, 0))/2, 0) * 0.15 +
