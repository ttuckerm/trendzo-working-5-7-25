...
-- FIXED: Numeric precision overflow issues
-- =====================================================
...
CREATE TABLE viral_recipe_book (
    ...
    effectiveness_score DECIMAL(4,3) DEFAULT 0, -- FIXED: 0.999 max
...
CREATE TABLE viral_video_gallery (
    ...
    viral_score DECIMAL(5,2) DEFAULT 0, -- FIXED: 999.99 max
...
CREATE TABLE video_framework_mapping (
    ...
    mapping_confidence DECIMAL(4,3) DEFAULT 0.95, -- FIXED: 0.999 max
...
CREATE TABLE prediction_validation (
    ...
    predicted_viral_score DECIMAL(5,2) NOT NULL, -- FIXED: 999.99 max
    actual_viral_score DECIMAL(5,2), -- FIXED: 999.99 max
    predicted_views INTEGER,
    actual_views INTEGER,
    validation_timestamp TIMESTAMP WITH TIME ZONE,
    accuracy_percentage DECIMAL(5,2), -- FIXED: 999.99 max
...
CREATE TABLE viral_dna_sequences (
    ...
    confidence_score DECIMAL(4,3) DEFAULT 0, -- FIXED: 0.999 max
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
CREATE TABLE template_generators (
    ...
    success_rate DECIMAL(4,3) DEFAULT 0, -- FIXED: 0.999 max
...
CREATE TABLE script_intelligence_data (
    ...
    viral_script_score DECIMAL(4,3) DEFAULT 0, -- FIXED: 0.999 max
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
CREATE TABLE viral_filters (
    ...
    effectiveness_rate DECIMAL(4,3) DEFAULT 0, -- FIXED: 0.999 max
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
CREATE TABLE evolution_engines (
    ...
    accuracy_improvement DECIMAL(4,3) DEFAULT 0, -- FIXED: 0.999 max
...
CREATE TABLE gene_taggers (
    ...
    tagging_accuracy DECIMAL(4,3) DEFAULT 0, -- FIXED: 0.999 max
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
-- =====================================================
-- POPULATE INITIAL DATA (Fixed precision values)
-- =====================================================

-- Insert initial data for Recipe Book (HOT templates) - FIXED values to be < 1.0
...
SELECT 
  video_id,
  framework_id,
  0.950, -- FIXED: Ensure this fits in DECIMAL(4,3)
  jsonb_build_object(
...
-- Insert prediction validation data with FIXED values that fit in DECIMAL(5,2)
...
SELECT 
    ...
    -- FIXED: Ensure actual_viral_score stays within reasonable bounds
    LEAST(99.99, GREATEST(0.01, viral_score + (random() * 4 - 2))) as actual_viral_score,
    ...
    -- FIXED: Ensure accuracy_percentage fits in DECIMAL(5,2)
    ROUND((90 + random() * 9)::numeric, 2) as accuracy_percentage,
